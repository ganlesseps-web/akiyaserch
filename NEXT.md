...
**未検証**: 実APIキーが手元に無いため、**実際のGemini疎通は未確認**(モック検証まで)。モデルIDやレスポンス形式が想定と違う可能性は残るので、キー設定後の初回実行でログ確認が必要(下記 Next (Mac))。
...
## Next (Mac)
- [ ] **【セッション16】Gemini APIキーを登録して AI判定を有効化** — 手順: ① https://aistudio.google.com/apikey でAPIキーを取得(無料枠) ② GitHubリポジトリの Settings → Secrets and variables → Actions で `GEMINI_API_KEY` という名前で登録 ③ `gh workflow run assess.yml --ref main` で初回実行 → ログで実際に判定できたか確認。**無料枠なので課金なし。** キー無しでもキーワード判定+自治体マスタは動くので必須ではない。
  - ⚠️ 実APIでの疎通は未検証(手元にキーが無いためモック検証まで)。初回実行時にモデルIDやレスポンス形式が想定と違えばログに出るので要確認。
  - ⚠️ 自分のアカウントの無料枠上限は https://aistudio.google.com/rate-limit で確認できる(公式は数値を非公開化しているため)。
