...
## Next (Mac)
- [ ] **【セッション15】AI構造化判定を有効にするか判断** — `assess.yml` は実装済みだが `ANTHROPIC_API_KEY` が GitHub Secrets に無ければ黙って skip する(現状スキップのはず)。有効化するなら https://console.anthropic.com/settings/keys でキー発行 → リポジトリ Secrets に `ANTHROPIC_API_KEY` を登録 → `gh workflow run assess.yml --ref main` で初回実行。コストは Haiku で1件あたり ~0.1円、初回全件でも数十円規模。**キー無しでもキーワード判定+自治体マスタは動く**ので必須ではない。
