...
- [ ] (中) **AI判定のスループットを上げるか判断** — 初回実行(80件)で429が一度も出ず、1番目のモデルだけで完走した = 無料枠に余裕がある。現状 `--limit 80` × `request_interval_seconds: 4.0` で1回6分52秒(timeoutは20分)。未判定が多いため全件終わるのに数日かかる。速めるなら assess.yml の `--limit` を 200 程度に、`config/ai.yaml` の待機秒を 2.0 程度に下げる余地あり(429が出てもフォールバックで安全に止まる設計)。急がないなら現状のまま毎朝少しずつでも問題なし。
