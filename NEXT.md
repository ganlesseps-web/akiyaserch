...
## Now
セッション12 (2026-07-12): 「0円物件も収集」要望に対応。調査で判明: みんなの0円物件(zero.estate)が React SPA + tRPC API に刷新され、旧 minna_0en が使っていた RSS(/feed/)が廃止 → **長期間0件で収集停止していた**(ローカルDBの price=0 は14件の古いデータのみ)。ブラウザで実 API を観測して公開エンドポイント `GET zero.estate/api/trpc/property.list`(superjson, {page,limit,sortBy,prefecture...})を特定し、minna_0en を全面書き換え。募集中のみ・建物あり(土地・建物/マンション/建物のみ)を収集、更地(土地のみ)は住む用途でないため除外。実測: 全1910件中1526件が成約済み、募集中は171件、うち建物ありは54件(対象13府県内7件)。全国対象だが件数少なく氾濫しないと確認。全99テスト緑(+6、_item_to_raw マッピングと 募集中/建物 フィルタを固定データで検証)。**本番反映**: claude/wip→main ff マージ→Vercel 自動デプロイ＋ scrape 手動起動で本番投入(下記)。**メモ**: 更地(土地のみ 117件)も欲しくなったら building_types に "土地のみ" を足す。0円は成約が速いので、売却済みが一覧に残る問題(sold検知)は今後の課題(Next参照)。
