...
## Next (Mac)
- [ ] **「住める空き家だけ」を本番反映 (セッション8)** — `claude/wip` → `main` に ff マージ → Vercel 自動デプロイ。これでボロ物件の非表示と「✨即入居OK」タブが本番に出る。
- [ ] **本番データに即入居判定を埋める** — 既存物件の move_in_ready 列は最初は空。本番 env で `trade reclassify` を1回実行すれば即埋まる。実行しなくても翌朝06:00 JST の自動巡回（再取得した物件を再判定）で順次埋まる。
